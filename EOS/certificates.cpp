...
class [[eosio::contract("greenmarket")]] greenmarket : public contract {
  ...
  public:
	...
	[[eosio::action]]
	void mint(uint8_t power) {
		require_auth( _self );
		eosio_assert( is_allowed_power(power), "available values is 0, 1, 2");
	...
	[[eosio::action]]
	void sale(name user, uint64_t certificate_id) {
		...
		eosio_assert(user.value == (*iterator).current_owner.value, "you are not owner of this sertificate" );
	...
	[[eosio::action]]
	void send(name user, uint64_t certificate_id) {
		require_auth( _self );
		eosio_assert( _self != user, "cannot transfer to self" );
		eosio_assert( is_account( user ), "account does not exist");

		certificates _certificates( _self, _self.value);
		auto certIterator = _certificates.find(certificate_id);
		eosio_assert(certIterator != _certificates.end(), "certificate with given id does not exist" );
